...
{   
    if(dir==0){
        if(m_city->nTootersAt(m_row-1,m_col)==0){    
            bool x = m_city->determineNewPosition(m_row,m_col,dir);
            if(x){
                m_age+=1;
            }
        }
...
{
    if (m_player->col()==c && m_player->row()==r){
        return true;
    }
    else{
        return false;
...
void City::display() const
{
    ...
    //indicate tooters position

    for(int i=0;i<m_nTooters;i++){
        char temp = grid[m_tooters[i]->row()-1][m_tooters[i]->col()-1];
        switch(temp){
            case '.':
...
{
      for(int i=0;i<m_nTooters;i++){
        int r = m_tooters[i]->row();
        int c = m_tooters[i]->col();
        if(isPlayerAt(r+1,c)){
            if(randInt(1,3)>1){
                Tooter * temp = m_tooters[i];
                for(int k=i;k<m_nTooters;k++){
                    m_tooters[k]=m_tooters[k+1];

                }
                m_tooters[m_nTooters]=temp;
                m_nTooters--;
            }
        }
        else if(isPlayerAt(r-1,c)){
            if(randInt(1,3)>1){
                Tooter * temp = m_tooters[i];
                for(int k=i;k<m_nTooters;k++){
                    m_tooters[k]=m_tooters[k+1];
...
void City::moveTooters()
{
    ...
    {
      m_tooters[k]->move();
      if(abs((m_tooters[k]->row()-m_player->row()))+abs((m_tooters[k]->col()-m_player->col()))==1){
        m_player->getGassed();
      }
